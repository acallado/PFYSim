...
def executeDblCycle(fob):  # Calculation.
    ...
    for move in fob.dbl_cycle_action_list:
        ...
        fob_spreader_position = calcSpreaderMove(
            fob, node_pos, fob_spreader_position, cnt_move_type, move_index)
        move_index += 1
...
def calcFOBMoveTime(distance):
    ...
    if distance < 2*fob_gantry_ramp_distance:
        time_gantry = 2*numpy.sqrt(
            distance/fob_gantry_ramp_acceleration)
    else:
        time_gantry = (
...
def calcSpreaderMove(fob, node_pos, spreader_pos, cnt_move_type, move_index):
    ...
    if cnt_move_type == 0:
        print(
            ...
            str(
                fob.dbl_cycle_action_list[move_index].
...
def setup():  # Calculation
    ...
    # Double stacking or simple stacking for inbounds
